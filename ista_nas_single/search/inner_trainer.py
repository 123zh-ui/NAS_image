...
#        self.criterion = nn.CrossEntropyLoss().cuda()
    ...
    def train_epoch(self, train_queue, epoch, all_freeze, alpha_list):
        losses = AverageMeter()
        top1 = AverageMeter()
        top5 = AverageMeter()
...
#           # alphas_normal表示的Z
            ...
            all_freeze = torch.as_tensor(all_freeze)
            scores, scores_aux = self.model(input, all_freeze.cuda())
            alpha_list.append([self.model.alphas_normal,self.model.alphas_reduce])
            alpha_len = len(alpha_list)
            #print("shape:",len(alpha_list))
            loss = F.cross_entropy(scores, target)#下面这个if的作用
            if self.auxiliary:
                loss_aux = F.cross_entropy(scores_aux, target)
                loss += self.auxiliary_weight * loss_aux
            loss.backward() #retain_graph=True
            self.alpha_optimizer.step()#结构系数的优化方法,后面具体看他的作用
#            self.w_optimizer.zero_grad()
...
#            loss.backward()
        ...
        if all_freeze:
            self.scheduler.step()
        return top1.avg, losses.avg, alpha_list
